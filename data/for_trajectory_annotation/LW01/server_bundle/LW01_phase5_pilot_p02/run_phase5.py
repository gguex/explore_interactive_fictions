    ...
    for task in TASKS:
        ...
        for row in rows:
            if task == "individual":
                ...
                validate_eligible_references(
                    row.get("eligible_profile_choice_refs"), story_text, identity
                )
            else:
                ...
                for side in ("story_a", "story_b"):
                    ...
                    validate_eligible_references(
                        story.get("eligible_profile_choice_refs"),
                        story_text,
                        f"{identity}/{side}",
                    )
...
def user_payload(task: str, row: dict[str, Any]) -> str:
    ...
    if task == "individual":
        value = {
            ...
            "eligible_profile_choice_refs": row[
                "eligible_profile_choice_refs"
            ],
        ...
        value = {
            ...
            "eligible_profile_choice_refs_a": row["story_a"][
                "eligible_profile_choice_refs"
            ],
            "story_a": row["story_a"]["story_text"],
            "trajectory_b_id": row["story_b"]["trajectory_id"],
            "eligible_profile_choice_refs_b": row["story_b"][
                "eligible_profile_choice_refs"
            ],
...
def eligible_references_from_story(story_text: str) -> list[str]:
    ...
    for match in block_pattern.finditer(story_text):
        ...
        if chosen is None:
            raise ValueError("Player-choice story block has no chosen-action reference")
        references.append(chosen.group(1))
    return references


def validate_eligible_references(value: Any, story_text: str, label: str) -> None:
    """Require one transferred allow-list to equal the story-derived list."""
    if not isinstance(value, list) or not all(isinstance(ref, str) for ref in value):
        raise ValueError(f"Invalid eligible profile-choice list: {label}")
    if len(value) != len(set(value)):
        raise ValueError(f"Duplicate eligible profile-choice reference: {label}")
    if value != eligible_references_from_story(story_text) or not value:
        raise ValueError(f"Eligible profile-choice list differs: {label}")
...
def validate_annotation_against_input(
    ...
    if task == "individual":
        ...
        _, _, paragraphs = valid_story_references(row["story_text"])
        choices = set(row["eligible_profile_choice_refs"])
            ...
            for field in (
                ...
                if isinstance(refs, list):
                    for reference in refs:
                        if reference not in choices:
                            errors.append(
                                f"profile_coherence cites ineligible profile evidence: "
                                f"{reference}"
                            )
    ...
    for side in ("a", "b"):
        ...
        steps, _, _ = valid_story_references(story["story_text"])
        choices = set(story["eligible_profile_choice_refs"])
        ...
        if isinstance(refs, list):
            for reference in refs:
                ...
                if reference not in valid:
                    errors.append(
                        f"ineligible or unknown story {side.upper()} evidence: "
                        f"{reference}"
                    )
    return errors
