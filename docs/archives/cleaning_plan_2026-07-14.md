...
1. (partiellement fait) Extraire les edges et les noeuds, avec plusieurs caractéristiques sur ces derniers choix. Cela a été fait de la manière suivante :
   ...
   - On bascule sur le cluster de calcul, où des scripts et un prompt (voir `cluster_scripts/`) sont utilisés avec un modèle local pour extraire des informations sur les edges. On rappatrie les résultats dans `results/phase1/LW01/curnagl/csv`. Plusieurs essais on été fait pour calibrer le prompt au mieux (les itérations inclues l'étapes suivante)
   - On regarde l'adéquation entre les résultats du cluster et le gold avec `scripts/utils/eval_diff.py`, les résultats sont stocké sous `results/phase1/LW01/curnagl/csv/rapport_erreurs_final.csv`. On garde un historique pour présenter les "progrès" effectués.
