...
\begin{document}
	...
	\end{frame}
  	...
  	\begin{frame}{Outline of the presentation}
  		...
  		\imp{2. Graph modeling and Bag-of-Paths formalism}
	...
	\begin{frame}{Rule-based parsing versus LLM-based annotations}
		...
		All remaining fields in the \imp{edge table}, in particular:
		...
		\begin{block}{\scriptsize \de{Methodological choice}}
			Each explicit player choice is annotated according to \imp{3 separate semantic axes}: 
	...
	\begin{frame}{LLM annotation: calibration and output fields}
		
		\scriptsize
		
		Same iterative procedure as in paragraph annotation, now applied to
		\imp{complete trajectories}.
		...
		\begin{tikzpicture}[
			...
			\node[calbox] (human) at (-3.7,0.55) {
				\textbf{Calibration dataset}\\[0.5mm]
				4/14 stories\\ 3/6 pairs (semantic axes $\times$ outcomes)
			};
			\node[calbox,fill=themecolor!7] (qwen) at (0,0.55) {
				\textbf{LLM annotation}\\[0.5mm]
				Qwen3.6-27B\\ Structured JSON
			};
			\node[calbox] (audit) at (3.7,0.55) {
				\textbf{Error analysis}\\[0.5mm]
				Comparison with human annotations\\
				Recurrent failures
			};
			\node[calbox,text width=2.9cm] (prompt) at (0,-0.95) {
				\textbf{Prompt}\\[0.5mm]
				Generic annotation rules
			};
			
			\draw[calarrow] (human.east) -- (qwen.west);
			\draw[calarrow] (qwen.east) -- (audit.west);
			\draw[calarrow] (prompt.north) -- (qwen.south);
			\draw[calarrow] (audit.south) |-
				node[pos=0.72,above,font=\tiny]{refine}
				(prompt.east);
		\end{tikzpicture}
		}
		...
		The 3rd prompt was frozen: 7/7 valid outputs, no inadmissible evidence, and 32/44 concordances. 
		...
		\begin{columns}[T,onlytextwidth]
			...
			\begin{column}{0.48\textwidth}
				\textbf{Individual trajectory}
				
				\begin{itemize}
					\setlength{\itemsep}{1pt}
					\item Perceived profile: levels of \texttt{risk}, \texttt{morality}, and
					      \texttt{action}.
					\item \texttt{profile\_coherence}
					\item \texttt{causal\_continuity}
					\item Choice references and short justifications
				\end{itemize}
			\end{column}
			...
			\begin{column}{0.48\textwidth}
				...
				\begin{itemize}
					\setlength{\itemsep}{1pt}
					\item Relative profile shift on \texttt{risk},
					      \texttt{morality}, and \texttt{action}
					\item \texttt{narrative\_distinctness}
					\item Evidence from stories A and B, with justification
					\item A/B and B/A orders used only to check order sensitivity.
				\end{itemize}
			\end{column}
	...
	\begin{frame}{Conclusion}
		...
		\begin{columns}[T,onlytextwidth]
			...
			\begin{column}{0.33\textwidth}
				\begin{exampleblock}{Next}
					...
					\begin{itemize}
						\setlength{\itemsep}{1pt}
						\item Refine the LLM annotation pipeline.
						\item Compare the results with a more powerful LLM.
						\item Make a contrastive study on different books.
						\item Refine modeling of the game mechanics (e.g. combat)
					\end{itemize}
		...
		\vspace{0.15cm}
		{		\footnotesize
			This contribution is a \imp{proof of concept for an auditable hybrid workflow} for analyzing gamebooks.  \\
			The LLM is used as a \imp{controlled annotator, not a literary critic}. \\
			BoP formalism provides \imp{several indices on gamebooks}.
		}	
