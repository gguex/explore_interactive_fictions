...
\institute{Section des sciences du langage et de l'information}
\titlegraphic{%
	\begin{tikzpicture}[remember picture,overlay]
		\node[anchor=south east,xshift=-0.45cm,yshift=0.4cm]
		at (current page.south east) {%
			\includegraphics[width=0.18\textwidth]{fig/unil_logo.pdf}%
		};
	\end{tikzpicture}%
}
...
\begin{document}
	...
	\begin{frame}{Interactive Fiction and Gamebook}
		\footnotesize
	...
	\end{frame}
  	...
  	\begin{frame}{A hybrid solution: graphs and LLMs}
  		\scriptsize
  		\imp{Large Language Models (LLMs)} make it possible to reintroduce the semantic content discarded by graph modeling. 
  	...
  	\begin{frame}{Methodological choice: local LLM}
  		...
  		\begin{columns}[T,onlytextwidth]
  			\begin{column}{0.48\textwidth}
  				\textbf{Auditable and independent}
  				\begin{itemize}
  					\item Local execution on university infrastructure keeps source
  					texts outside proprietary APIs.
  					\item Prompts, inference settings, and outputs can be stored,
  					inspected, and reproduced.
  				\end{itemize}
  			\end{column}
  			\begin{column}{0.48\textwidth}
  				\textbf{Appropriate to the task}
  				\begin{itemize}
  					\item Annotation categories and admissible evidence are defined
  					by an explicit codebook and JSON schema.
  					\item A constrained annotation task does not require the
  					generative capabilities of a frontier model.
  				\end{itemize}
  			\end{column}
  		\end{columns}
  	...
  	\begin{frame}{Corpus and modeling scope}
  		\footnotesize
  		\imp{Project Aon} provides authorized, HTML-structured editions of Joe
  		Dever's \emph{Lone Wolf} series. This proof of concept studies only
  		\imp{Book 1: \emph{Flight from the Dark}}.
  		
  		\vspace{0.2cm}
  		
  		The objective is a tractable \imp{Markovian model}: once the profile is
  		fixed, the next transition depends on the current paragraph rather than
  		the complete play history.
  		...
  		\begin{center}
  			...
  			\begin{tabular}{|c|c|p{0.43\textwidth}|c|}
  				\hline
  				\textbf{Level} & \textbf{Information} & \textbf{Mechanics} &
  				\textbf{Decision} \\
  				\hline
  				\textbf{L0} & Topology &
  				...
  				\hline
  				\textbf{L1} & Player agency &
  				Choices annotated along the risk, morality, and action axes &
  				\textcolor{green!50!black}{\textbf{Included}} \\
  				\hline
  				\textbf{L2} & Uncertainty &
  				Random events, combat, evasion, Kai Disciplines, and conditions &
  				\textcolor{orange!80!black}{\textbf{Simplified}} \\
  				\hline
  				\textbf{L3} & Persistent state &
  		...
  		\scriptsize
  		L2 uses explicit, global probabilities. Their values are reported when
  		the profile-dependent graph is introduced.
  	\end{frame}
  	...
  	\begin{frame}{Objective: from paragraphs to structured data}
  		...
  		\begin{block}{\footnotesize \de{Objective}}
  			\footnotesize
  		...
  		\vspace{0.2cm}
  		{\footnotesize
  			Each explicit choice is annotated on three independent axes:
  			\imp{risk} (cautious--reckless),
  			\imp{morality} (selfish--noble), and
  			\imp{action} (physical--tactical); each axis also allows a neutral value.
  		}
	...
	\begin{frame}{Annotation results}
		...
		missing or invented transition and no transition-type disagreement. Only 3 (of 177 annotations) semantic disagreements: two for risk and one for action.
	...
	\begin{frame}{Result of paragraph annotation : unweighted graph}
		
		\begin{center}
			\includegraphics[width=\textwidth]{fig/LW01_phase1_unweighted_graph.png}
		\end{center}
		
		\vspace{-0.15cm}
		
		{\small
			The topology and transition types are now available, but \imp{transition probabilities depend on player profiles} (next section).
		}
	...
	\begin{frame}{Objective: from annotations to structural measures}
		...
		\vspace{0.1cm}
		\scriptsize
		\begin{block}{\scriptsize \de{Objectives}}
			\begin{itemize}
				...
				\item Compute \imp{various indices with the Bag-of-Paths formalism}.
			\end{itemize}
		\end{block}
		...
		\begin{itemize}
			\item How can game mechanics be converted into transition probabilities?
			\item How does the resulting graph vary across player profiles?
			\item Which parts are most important across all possible trajectories?
		\end{itemize}
	...
	\begin{frame}{One Markov chain per player profile}
		...
		transition depends only on the current paragraph (Markov chain).
			...
			\begin{column}{0.48\textwidth}
				...
				{\scriptsize
					...
					\begin{itemize}
						...
						\item Expected trajectory length and entropy
	...
	\begin{frame}{Objective: from probabilistic paths to complete stories}
		...
		\begin{block}{\footnotesize \de{Objective}}
			See if player profiles become perceptible in \imp{complete stories}.
		\end{block}
		...
		\raggedright
		A \imp{medoid} is an observed trajectory that minimizes its average distance to all other sampled trajectories, making it the most central representative path.
	
		The model infers whether the intended profile becomes perceptible from the
		\imp{story and choices only}. The hidden metadata and BoP measures are reintroduced only for the final comparison.
		
		{\scriptsize
			\textbf{Calibration:} the phase-1 refinement loop was reused on 4/14
			stories and 3/6 pairs. The third generic prompt was frozen after 7/7 valid
			outputs, with 32/44 human--model concordances.
		}
	...
	\begin{frame}{Individual annotation results}
		
		\begin{center}
			\includegraphics[width=\textwidth]{fig/01_individual_trajectories.png}
			...
			{\scriptsize
				...
				\begin{tabular}{ccc}
					\hline
					\textbf{Causal continuity} & \textbf{Profile coherence} &
					\textbf{Action recovery by level} \\
					\hline
					14/14 & 9/14 &
					Neutral 0/10; Physical 0/2; Tactical 2/2 \\
					\hline
				\end{tabular}
			}
		\end{center}
	...
	\begin{frame}{Pairwise annotation results}
		
		\small
		
		\begin{center}
			\includegraphics[width=0.96\textwidth]{fig/02_trajectory_comparisons.png}
		\end{center}
		 
		\vspace{-1.3cm}
		
		{\scriptsize \textbf{Reading:} bold = controlled axis; red = A/B--B/A disagreement averaged across orders, "main tendency" output displayed.}
		
		\vspace{0.3cm}
		
		Intended profile contrast recovered in \imp{5 of 6} comparisons.
	...
	\begin{frame}{Conclusion}
		...
		\begin{columns}[T,onlytextwidth]
			...
			\begin{column}{0.32\textwidth}
				\begin{block}{What we learned}
					...
					\begin{itemize}
						...
						\item Profiles reshape the probabilistic structure.
						\item Risk dominates the other axes.
						\item 5/6 relative story contrasts are recovered; absolute profiles
						remain unevenly visible.
					\end{itemize}
				\end{block}
			\end{column}
			...
			\begin{column}{0.32\textwidth}
				\begin{alertblock}{Current limits}
					...
					\begin{itemize}
						\setlength{\itemsep}{1pt}
						\item One gamebook only.
						\item Simplified mechanics and state for the Markov model.
						\item Limited human calibration; no held-out validation.
					\end{itemize}
				\end{alertblock}
			\end{column}
			...
			\begin{column}{0.33\textwidth}
				\begin{exampleblock}{Next}
					...
					\begin{itemize}
						\setlength{\itemsep}{1pt}
						\item Test the workflow comparatively on other books.
						\item Add richer game-state mechanics and independent human validation.
					\end{itemize}
		...
		\vspace{0.15cm}
		{\footnotesize
			This is a \imp{proof of concept for an auditable hybrid workflow}:
			BoP measures narrative structure, while the local LLM remains a
			controlled annotator rather than a literary critic.
		}
	...
	\begin{frame}[plain]
		\centering
		\vfill
		{\Huge\bfseries\color{themecolor}Thank you for your attention!}
		\vfill
	\end{frame}
